...
class DeploymentRequest(BaseModel):
    ...
    namespace: str = Field(
        ..., min_length=1, max_length=63, description="Kubernetes namespace"
    )
    docker_image: str = Field(
        ..., description="Docker image to deploy (must be from whitelist)"
    )
    has_storage: bool = Field(
        default=False, description="Whether to provision persistent storage"
    )
    has_database: bool = Field(
        default=False, description="Whether to setup database connection"
    )

    # Optional fields (for metadata/logging only - not used in deployment)
    memory_request: Optional[int] = Field(
        default=256, description="Memory request in MB (ignored, always set to 256)"
    )
    memory_limit: Optional[int] = Field(
        default=512, description="Memory limit in MB (ignored, always set to 512)"
    )
    ...
    cpu_limit: Optional[int] = Field(
        default=500, description="CPU limit in millicores (ignored, always set to 500)"
    )
    storage_size: Optional[int] = Field(
        default=10, description="Storage size in GB (ignored, always set to 10)"
    )
...
class DeploymentResponse(BaseModel):
    ...
    cleanup_at: datetime = Field(
        ..., description="ISO timestamp when deployment will be deleted"
    )
...
class User(BaseModel):
    ...
    allowed_namespaces: List[str] = Field(
        default_factory=list, description="List of namespaces user can access"
    )
