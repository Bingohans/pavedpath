            ...
            try:
                ...
                for key, data in to_cleanup:
                    ...
                    try:
                        ...
                        from main import argocd_client, github_client
                        ...
                        if data.get("cleanup_argocd") and argocd_client:
                            ...
                            try:
                                ...
                                logger.info(f"ArgoCD application deleted: {pod_name}")
                            except Exception as e:
                                logger.error(f"ArgoCD cleanup failed: {e}")

                        # 2. THEN delete GitHub repository
                        ...
                        # 3. Fallback K8s cleanup (hvis ArgoCD fejler)
                        logger.info(f"Verifying Kubernetes cleanup: {key}")
                        try:
                            self.k8s_client.delete_pod(namespace, pod_name)
                        except Exception as e:
                            logger.debug(
                                f"K8s cleanup not needed (already deleted by ArgoCD): {e}"
                            )
